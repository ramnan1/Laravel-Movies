...
>
  <input wire:model="search" type="text" class="bg-gray-800 text-sm rounded-full w-64 px-4 pl-8 py-1 focus:outline-none focus:ring focus:ring-blue-500" placeholder="Search (Press '/' to focus)"
  x-ref="search"
  @keydown.slash.prevent.window="$refs.search.focus()"
  ...
  @if ( strlen($search) > 2 )
    ...
    x-show="isOpen " x-transition.opacity.duration.500ms
        ...
        <ul>
          @foreach ($searchResults as $result)
            <li class="border-b border-gray-700">
              <a
              href="{{ route('movie.show', $result['id']) }}" class="hover:bg-gray-700 px-3 py-3 flex items-center"
